...
def test():
    """Автоматическое тестирование функции main."""
    # 1. Базовый тест: единичные матрицы 3×3
    I1 = np.eye(3)
    res1 = main(I1, I1, I1, I1)
    expected1 = np.sqrt(3)   # ≈ 1.7320508075688772
    assert np.isclose(res1, expected1, rtol=1e-9), f"{res1} != {expected1}"
    # Тест 1 пройден (единичные матрицы).
    ...
    expected2 = 0.0
    assert np.isclose(res2, expected2, atol=1e-12), f"{res2} != {expected2}"
    # Тест 2 пройден (нулевые матрицы).
    ...
    # Тест 3 пройден (10 случайных наборов матриц).

    # 4. Проверка, что функция возвращает скаляр, а не массив
    I2 = np.eye(2)
    res4 = main(I2, I2, I2, I2)
    assert isinstance(res4, (float, np.floating)), f"Не скаляр: {type(res4)}"
    # Тест 4 пройден (тип возвращаемого значения).

    # Все тесты успешно завершены.

# Следующее только для локальных тестов, перед отправкой на сайт необходимо удалить
